...
int main(){
    ...
    */
    ...
    float pi,pi2, kankaku,hatyou,goukei;
...
/*
    for(i=0;i<HEI;i++){
	 ...
	 dx=(float)(x[k]-j);
	 dy=(float)(y[k]-i);
	     tmp=tmp+cos(goukei*0.5F*(dx*dx+dy*dy)/z[k]);
       //tmp=tmp+cos(goukei*sqrt(dx*dx+dy*dy+z[k]*z[k]));
       //img_buf[i*WID+j]=img_buf[i*WID+j]+cos(goukei*sqrt((x[k]-j)*(x[k]-j)+(y[k]-i)*(y[k]-i)+z[k]*z[k]));
        }
      img_buf[i*WID+j] = tmp;
      }
    }
  ...
  for(i=0;i<HEI;i++){
    for(j=0;j<WID;j++){
      ...
      for(k=0;k<tensuu;k++){
        dx=(float)(x[k]-j);
     	  dy=(float)(y[k]-i);
        tmp=cos(goukei*sqrt(dx*dx+dy*dy+z[k]*z[k]));
        hr+=cosf(tmp);
        hi+=sinf(tmp);
      }
      tmp_h=atan2f(hi,hr);
